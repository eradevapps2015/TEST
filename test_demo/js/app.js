...
//    
    ...
    $rootScope.appServerIp='http://10.11.201.214:8084/'
...
//	}
	...
	$scope.login = function (user) {
		...
		$ionicLoading.show();
            $http({
              method: 'POST',					 
              url: $rootScope.appServerIp+'BTSDEMOSERVICE/AppLoginService',
              params: {uname:user.uname, pass:user.pass, appVersion:'BTS_DEMO1.0'},
              type:'JSON'
              //, headers : { 'Content-Type': 'application/json' }
                ...
                if(data.userId != null || data.userId != undefined) {                    
                    ...
                    $rootScope.userCompanyCode = data.userCompanyCode;                    
                    $rootScope.userBranchList = data.userBranchList;
                    //alert(data.userName);
...
.controller('SignIn2Ctrl', function($scope, $state, $http, $rootScope, $ionicLoading, $timeout) {
    
    $rootScope.appServerIp='http://10.11.201.214:8084/'
    document.getElementById("company").innerHTML = $rootScope.userCompanyCode;
    
    $rootScope.responseArr = [];
    angular.forEach($rootScope.userBranchList, function(userBranchList, index) {
        $ionicLoading.hide();
        $rootScope.responseArr.push(userBranchList);
    });
   /*
	$scope.login2 = function (user) {
		...
		$ionicLoading.show();
					$http({
					  ...
					  url: $rootScope.appServerIp+'BankAndroidConnectivity/LoginMobiBank',
					  params: {uname:user.uname, pass:user.pass, appVersion:'BTS_DEMO1.0'},
					  //type:'JSON',
					  headers : { 'Content-Type': 'application/json' }
					}).success(function(data, status, headers, config) {
